...
const UI_VERSION='v2.0.16';
const MODERN_BUILD='2026-09-22-v2.0.16-share-links-1';
...
async function verifyBuild(){
  ...
  try{
    ...
    if(proof){proof.textContent=modern?'✓ v2.0.16 · SHARED CORE VERIFIED':'✓ LEGACY v8.9.3 VERIFIED';proof.classList.add('ok');}
    return true;
  }catch(e){
...
function hasSharedSettingsHash(){
  const params=new URLSearchParams(location.hash.replace(/^#/,''));
  return !!(params.get('s')||params.get('share'));
}
async function decodeSharedSettingsFromHash(){
  const params=new URLSearchParams(location.hash.replace(/^#/,''));
  const compactRaw=params.get('s');
  const legacyRaw=params.get('share');
  if(!compactRaw&&!legacyRaw) return null;
  return compactRaw?await decodeCompactSharePayload(compactRaw):decodeLegacySharePayload(legacyRaw);
}
function clearShareHash(){
  if(!hasSharedSettingsHash()) return;
...
async function applySharedSettingsFromHash({announce=false}={}){
  if(!hasSharedSettingsHash()) return false;
  try{
    const settings=await decodeSharedSettingsFromHash();
    if(!settings||!applySettings(settings)) throw new Error('공유 설정을 적용하지 못했습니다.');
    sharedViewActive=true;
    setSaveStatus('공유 설정 임시 보기','warn');
    if(announce) showShareToast('친구 설정을 임시로 불러왔습니다. 내 설정은 그대로 보관됩니다.','ok');
    return true;
  }catch(e){
...
async function restoreOwnSettingsView({announce=false}={}){
  sharedViewActive=false;
  await loadSettings();
  if($('#employmentStartYear')&&!String($('#employmentStartYear').value||'').trim()) $('#employmentStartYear').value=String(new Date().getFullYear()+5);
  renderCurrentSettingsState();
  setSaveStatus('내 설정','saved');
  runSimulation();
  if(announce) showShareToast('내 설정으로 돌아왔습니다.','ok');
}
async function handleShareNavigation(){
  if(hasSharedSettingsHash()){
    const applied=await applySharedSettingsFromHash({announce:true});
    if(applied){
      renderCurrentSettingsState();
      runSimulation();
    }else{
      await restoreOwnSettingsView();
    }
  }else if(sharedViewActive){
    await restoreOwnSettingsView({announce:true});
  }
}
function bindSharing(){
  $('.share-settings').forEach(btn=>btn.addEventListener('click',shareCurrentSettings));
  window.addEventListener('hashchange',()=>{handleShareNavigation().catch(e=>showShareToast('설정 전환 오류: '+e.message,'bad'));});
}
...
function updateCashflowHint(){
  ...
  const employmentYear=Math.round(+$('#employmentStartYear').value||year);
  const employed=year>=employmentYear,currentSalary=employed?income*Math.pow(1+growth/100,Math.max(0,year-employmentYear)):0;
  const contrib=currentContributionManwonForYear(year),base=currentSalary-living-contrib,el=$('#cashflowHint');
  if(!employed){
    el.textContent=`${employmentYear}년 취업 예정 · 취업 전 월급은 0원으로 계산합니다. 초봉 ${income.toLocaleString('ko-KR')}만원, 이후 연 ${growth>=0?'+':''}${growth.toFixed(1)}% 복리 상승.`;
    el.className='hint';
  }else if(base<0){
    el.textContent=`⚠ ${year}년 기준 월급에서 생활비·납입을 빼면 ${manwon(manwonInput(-base))} 부족합니다. Cashflow를 켜면 배당까지 포함한 상세 흐름을 볼 수 있습니다.`;el.className='hint bad';
  }else{
    el.textContent=`${year}년 월급 - 생활비 - 납입 = ${manwon(manwonInput(base))}/월 · ${employmentYear}년 취업, 연봉 ${growth>=0?'+':''}${growth.toFixed(1)}% 복리 반영.`;el.className='hint';
  }
}
['income','salaryGrowth','employmentStartYear','fireexp'].forEach(id=>$('#'+id).addEventListener('input',()=>{updateCashflowHint();updateFoldSummaries();}));
...
function drawChart(container,rows,series,fireMonth=null,cashMode='withdrawal',showCashflow=false){
  ...
  function renderRow(r,idx,clientX=null,clientY=null,showTip=true){
    ...
    const employmentStartYear=Number(r.employmentStartYear)||Number(r.calendarYear)||0;
    const employmentNote=Number(r.calendarYear)<employmentStartYear?`${employmentStartYear}년 취업 예정 · 연 ${salaryGrowth>=0?'+':''}${(salaryGrowth*100).toFixed(1)}%`:`연 ${salaryGrowth>=0?'+':''}${(salaryGrowth*100).toFixed(1)}%`;
    ...
    const cashflowBody=showCashflow?`<div class="tooltip-cashflow">
      ...
      <div class="tooltip-cashflow-breakdown">
        <span><em>월급</em><b>${compactKRW(salary)}</b><small>${employmentNote}</small></span>
    ...
    const cashflowReadout=showCashflow?`<b class="readout-cashflow">Cashflow ${compactKRW(totalCash)}/월 = 월급 ${compactKRW(salary)} (${employmentNote}) + 세후배당 ${compactKRW(dividend)}${fireOther>0?` + FIRE 후 기타소득 ${compactKRW(fireOther)}`:''}</b>`:'';
...
async function boot(){
  ...
  await loadSettings();
  if($('#employmentStartYear')&&!String($('#employmentStartYear').value||'').trim()) $('#employmentStartYear').value=String(new Date().getFullYear()+5);
  await applySharedSettingsFromHash({announce:hasSharedSettingsHash()});
  renderCurrentSettingsState();
  bindAutoSave();
  bindSharing();
  if(sharedViewActive) setSaveStatus('공유 설정 임시 보기','warn');
  else scheduleSave();
  runSimulation();
}
boot();

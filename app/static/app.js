...
const UI_VERSION='v2.0.7';
const MODERN_BUILD='2026-09-20-v2.0.7-new-user-prefill-1';
...
async function verifyBuild(){
  ...
  try{
    ...
    if(proof){proof.textContent=modern?'✓ v2.0.7 · SHARED CORE VERIFIED':'✓ LEGACY v8.9.3 VERIFIED';proof.classList.add('ok');}
    return true;
  }catch(e){
...
async function boot(){
  await verifyBuild();
  await loadSettings();
  if($('#employmentStartYear')&&!String($('#employmentStartYear').value||'').trim()) $('#employmentStartYear').value=String(new Date().getFullYear());
  renderPF();

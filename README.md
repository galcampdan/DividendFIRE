...
- 공유 링크를 열면 친구 설정이 **즉시 임시 적용**되지만, 받는 사람의 저장된 내 설정은 덮어쓰지 않습니다. 공유 링크에서 값을 수정해도 내 설정에는 저장되지 않습니다.
- 공유 링크에는 현재 나이, 투자자산, 월급 등 입력한 값이 포함될 수 있으므로 공개 게시 시 주의합니다.
- 브라우저 뒤로가기로 공유 해시가 사라지거나 원래 Web/PWA 주소로 다시 들어가면 저장돼 있던 내 설정을 복원합니다.
